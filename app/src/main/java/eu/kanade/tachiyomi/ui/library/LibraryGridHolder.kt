...
import eu.kanade.tachiyomi.data.coil.onLibraryImageError
...
//                }
            ...
            onError = { cover.onLibraryImageError() },
            onSuccess = {
                val hasRatio = binding.coverThumbnail.layoutParams.height != ViewGroup.LayoutParams.WRAP_CONTENT
                val ratio = MangaCoverMetadata.getRatio(manga)
                if (!fixedSize && !hasRatio && ratio != null) {
                    setFreeformCoverRatio(ratio)
                }
            }
        )
    }
    ...
    fun setFreeformCoverRatio(ratio: Float?, parent: AutofitRecyclerView? = null) {
        binding.setFreeformCoverRatio(ratio, parent)
    }
...
fun MangaGridItemBinding.setFreeformCoverRatio(manga: Manga?, parent: AutofitRecyclerView? = null) {
    setFreeformCoverRatio(manga?.let { MangaCoverMetadata.getRatio(it) }, parent)
}

fun MangaGridItemBinding.setFreeformCoverRatio(ratio: Float?, parent: AutofitRecyclerView? = null) {
    val itemWidth = parent?.itemWidth ?: root.width
    if (ratio != null) {
        coverThumbnail.ratio = ratio
        ...
        val coverHeight = itemWidth / 3f * 4f
        coverThumbnail.ratio = itemWidth / coverHeight
        constraintLayout.minHeight = coverHeight.toInt() / 2
